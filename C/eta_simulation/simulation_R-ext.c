...
int
Evolve_Sequence(struct seq_struct *seq, int aa_count, int seq_num,
        double *RESIDENCE_TIMES_SIM_GENOTYPES, double * ETA_SIM_GENOTYPES,
        double * MU_RATIO_SIM_OVER_OBS, 
        double *UNSCALED_PR_OBS_GENOTYPE, double *ETA_REF, double *bin_lims,
        double *BINS, int *NUM_BINS, double *BURN_IN_ETA, double *BURN_IN_TIME)
{
    ...
    double seq_mu = 0;
    double step_mu = 0;
    double delta_eta_0_i = *ETA_REF - seq->eta_initial; //This seems backwards from standard convention, ````
    double step_eta;
...
/*========================================================*
 *              4) START SEQ ITERATIONS                   *
 *========================================================*/

    evol_steps = 0;
    time_step = 0;
    
    //for (evol_time = 0; evol_time < max_time; evol_time += time_step) { //this is for fixed time, for now we want fixed steps
    for (evol_steps = 0; evol_steps < total_evol_steps; evol_steps++) {
        ...
        //evol_steps++; needed this when we had fixed time
        
        //*=======================================*
        //*  4.a) Calculate Mu Fitness product to *
        //*  fill in UNSCALED_PR_SIM_GENOTYPES    *
        //*  Do this before making substitution   *
        //*  so that time_step corresponds to the *
        //*  correct sequence                     *
        //*=======================================*
         
        if(*ETA_REF == 0) step_fitness_Ne = exp(-Q * Ne * phi * (seq->eta_obs - seq->eta_initial));
            else step_fitness_Ne = exp(-Q * Ne * phi * (seq->eta_obs-*ETA_REF));    //fitness raised to Ne. 
      
            //* record current eta value for later comparision to the replacement eta and delta_eta
        previous_eta= seq->eta_obs;

        step_eta = seq->eta_obs;    
        step_mu = Calc_Seq_Mu(seq->codon_cts, seq->aa_count);
        step_mu_fitness_Ne = step_mu*step_fitness_Ne;   //this goes into UNSCALED_PR_SIM_GENOTYPES
        ...
        if(!random_walk){
            ...
            //If random walk flag, set all replacement pr's to 1
            for(i=0;i<D_dim;i++){
                *(pr_vec + i) = 1;
            }
            pr_total = D_dim;
        ...
        if (benchmark||non_random_time_step){
            ...
            //* think about it more.
            //* The problem is D_Array set up kinda screwy so #FFFFFF#4747EAthat one may need to shift
            //* everything up or down depending on the # of neighbors of the mutant
            //* Further, lots of stuff needs to be rescaled
            //*
        
        //* Bin eta, weighted by time_step
        if(evol_steps >= burn_in_steps){
			bin_eta(seq->eta_obs,BINS,bin_lims,time_step,*NUM_BINS);    
		}
        ...
        if(directly_update_eta){
            ...
            Convert_Codon_Index_to_Sigma_Vec (&(seq->codon_index[0]),
                            &(seq->sigma_vec[0]),
                              aa_count);
            new_xi = Calc_Xi_NSE (&(seq->sigma_vec[0]), aa_count);
            new_sigma_n = seq->sigma_vec[aa_count - 1];
            new_eta = Calc_Eta_NSE (new_sigma_n, new_xi, aa_count);
        ...
        seq->evol_delta_eta_var = evol_delta_eta_var;
            ...
            //* keep sum of delta_eta_0_i this is the delta eta for the current sequence minus the original
            //* \eta_mut - \eta_wt
        delta_eta_0_i += delta_eta;
        
        //*=======================================*
        //* 4.j) Import into vectors passed to R  *
        //* Do not begin importing values until # *
        //* required burn in steps are taken.     *
        //* Only burn in on first iteration       *
        //*=======================================*
        
        if(*ETA_REF == seq->eta_initial){
            
            if(evol_steps >= burn_in_steps){
                //*(UNSCALED_PR_SIM_GENOTYPES + evol_steps - burn_in_steps) = step_fitness_Ne;
                            
                *(RESIDENCE_TIMES_SIM_GENOTYPES + evol_steps - burn_in_steps) = time_step;
                
                *(ETA_SIM_GENOTYPES + evol_steps - burn_in_steps) = step_eta;
                
                *(MU_RATIO_SIM_OVER_OBS + evol_steps - burn_in_steps) = step_mu;
            }else{
                *(BURN_IN_ETA + evol_steps) = step_eta;
                *(BURN_IN_TIME + evol_steps) = time_step;
            }
            
        }else if(*ETA_REF!=seq->eta_initial) {
            //*(UNSCALED_PR_SIM_GENOTYPES + evol_steps) = step_fitness_Ne;
            
            *(RESIDENCE_TIMES_SIM_GENOTYPES + evol_steps) = time_step;
            
            *(ETA_SIM_GENOTYPES + evol_steps) = step_eta;
            
            *(MU_RATIO_SIM_OVER_OBS + evol_steps) = step_mu;
        }
...
void MCMC_Sequence(struct seq_struct *seq, int aa_count, int seq_num,
        double *RESIDENCE_TIMES_SIM_GENOTYPES, double * ETA_SIM_GENOTYPES,
        double * MU_RATIO_SIM_OVER_OBS, 
        double *UNSCALED_PR_OBS_GENOTYPE, double *ETA_REF, double *bin_lims,
        double *BINS, int *NUM_BINS, double *BURN_IN_ETA, double *BURN_IN_TIME){
    ...
    //3: Start for loop-- start num substitutions at 1
    
    //Initialize tSeq.codon_index
    for(i=0;i<seq->aa_count;i++){
		tSeq.codon_index[i] = seq->codon_index[i];
	}
    
	while(accept_num < total_evol_steps) {
		
		
    //4: Take a step in codon space
		for(i=0;i<2*num_sub;i++){
			*(randNums + i) = gsl_rng_uniform(rn);
		}
		
		flip_codons(seq,&(tSeq),randNums,num_sub);
    //5: Calc new eta and mu
		Convert_Codon_Index_to_Sigma_Vec(tSeq.codon_index,
                     tSeq.sigma_vec,
                     aa_count);

		tSeq.sigma_obs = tSeq.sigma_vec[aa_count - 1];

		tSeq.xi_obs = Calc_Xi_NSE(tSeq.sigma_vec, aa_count);

		tSeq.eta_initial = tSeq.eta_obs = Calc_Eta_NSE(tSeq.sigma_obs, tSeq.xi_obs, aa_count);
		
		Codon_Counts(tSeq.codon_index,tSeq.codon_cts,tSeq.aa_count);
		
		tSeq.mu_obs = Calc_Seq_Mu(tSeq.codon_cts,tSeq.aa_count);
		
    //6: accept pr = min(w(new)/w(old),1)
		accept_pr = fmin(tSeq.mu_obs/seq->mu_obs*exp(-Q*Ne*seq->phi_obs*(tSeq.eta_obs-seq->eta_obs)),1);
		
		Rprintf("accept_pr: %f ",accept_pr);
    //7: Draw rn and accept/reject
		...
		*randNums = gsl_rng_uniform(rn);
		...
		if(*randNums < accept_pr){
			...
			if(*ETA_REF == seq->eta_initial){ //If this is a new step, add it to eta vector
            
				if(accept_num >= burn_in_steps){
					//*(UNSCALED_PR_SIM_GENOTYPES + accept_num - burn_in_steps) = step_fitness_Ne;
                            
					*(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num - burn_in_steps) = 1;
                
					*(ETA_SIM_GENOTYPES + accept_num - burn_in_steps) = step_eta;
                
					*(MU_RATIO_SIM_OVER_OBS + accept_num - burn_in_steps) = step_mu;
				}else{
					*(BURN_IN_ETA + accept_num) = step_eta;
					*(BURN_IN_TIME + accept_num) = 1;
				}
            
			}else if(*ETA_REF!=seq->eta_initial) {
				//*(UNSCALED_PR_SIM_GENOTYPES + accept_num) = step_fitness_Ne;
            
				*(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num) = 1;
            
				*(ETA_SIM_GENOTYPES + accept_num) = step_eta;
            
				*(MU_RATIO_SIM_OVER_OBS + accept_num) = step_mu;
			}
			...
			if(*ETA_REF == seq->eta_initial){
				...
				if(accept_num >= burn_in_steps){
					t = *(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num - burn_in_steps);
          
				    t++;
				  
			        *(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num - burn_in_steps) = t;
				    Rprintf("*(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num - burn_in_steps) %d\n",*(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num - burn_in_steps));
				  
				}else{
					
					*(BURN_IN_TIME + accept_num) = *(BURN_IN_TIME + accept_num)+1;
				}
            
			}else if(*ETA_REF!=seq->eta_initial) {
				//*(UNSCALED_PR_SIM_GENOTYPES + accept_num) = step_fitness_Ne;
				*(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num) = *(RESIDENCE_TIMES_SIM_GENOTYPES + accept_num)+1;
        
			}
			step_num++;
		}
    
    //9: adjust num_substitutions
    //      -if(accept ratio > desired_acc_ratio): increase num substitutions
    //      -if(accept ratio < desired_acc_ratio): decrease num substitutions
		accept_ratio = (double) accept_num/(double)step_num;
...
//void simulate_eta(int *CODON_INDEX,                  double *ELONGATION_RATES,         double *MUTATION_RATES, //RENAME TO THIS
void CES(int *CODON_INDEX,                  double *ELONGATION_RATES,         double *MUTATION_RATES, 
         int *AA_COUNT,                     double *PHI,                      double *RESIDENCE_TIMES_SIM_GENOTYPES, 
         double * ETA_SIM_GENOTYPES,        double * MU_RATIO_SIM_OVER_OBS,   double *UNSCALED_PR_OBS_GENOTYPE,  
         double *ETA_REF,                   int *CODON_COUNTS_REF,            double *POP,
         double *A_1,                       double *A_2,                      double *AT_BIAS,                  
         int *BENCH,                        double *BEE,                      double *GMT,                      
         int *IGNORE,                       int *RANDOM,                      double *GAMMA,                    
         double *SCALE_FACTOR,              int *MES,                         int *BIS,                         
         double *ETA_MIN,                   double *ETA_MAX,                  double *BINS,                     
         int *NUM_BINS,                     double *BURN_IN_ETA,              double *BURN_IN_TIME,           
         char ** AA_VEC,                    char ** CODON_VEC,                int * N_CODON)
{
    ...
    for(i = 0;i < 64;i++){
        Sequence.codon_cts[i] = *(CODON_COUNTS_REF + i);
    }
    ...
    }else total_evol_steps = simulation_steps;
    ...
    //Codon_Counts(); //Set initial Codon counts-- leaving this out should give the differences in codon counts from
    //current evolution step minus original sequence
    //for(i=0;i<64;i++){
    //  Seq.codon_cts[i] = 0;
    //}
    //If we don't initialize this on every run, it should keep the codon counts from 
    //the previous run stored in the same memory location, which can be continuously 
    //incremented to give codon counts relative to the original sequence
    //NOTE: this method caused things to get screwy, so we now read in the codon counts from
    //      the previous run via CODON_COUNTS_REF

    //calculate mu_obs for initial sequence
    //To be completed. Need to understand information in AA[i].mu[][] matricies
    //jmax = Sequence.aa_count;
    //mu_obs = 1;
    //for(j = 1; j<jmax; j++){
    //  codon_index = Sequence.codon_index[j];
    //  mu_obs*= Codon[codon_index].syn_relative_mutation_rate
    //}
    //Sequence.mu_obs = ;
    ...
    Convert_Codon_Index_to_Sigma_Vec(Sequence.codon_index,
                     Sequence.sigma_vec,
                     aa_count);

    Sequence.sigma_obs = Sequence.sigma_vec[aa_count - 1];

    Sequence.xi_obs = Calc_Xi_NSE(Sequence.sigma_vec, aa_count);

    Sequence.eta_initial = Sequence.eta_obs =
        Calc_Eta_NSE(Sequence.sigma_obs, Sequence.xi_obs,
             aa_count);
    
    if (random_start&&*ETA_REF == 0) {
        //replace codon sequence and index  with a randomly generated one
        //IF random start flag is switched on and we have not already burned in.
        Generate_Random_Codon_Index(Sequence.codon_index,
                            Sequence.aa_index,
                            aa_count);
    }
    
    //*===========================================*
    //* 4) If first set of samples in simulation, *
    //*    set *ETA_REF                           *
    //*===========================================*
    
    if(*ETA_REF == 0) {
        *ETA_REF = eta_original = Sequence.eta_initial;
    }else{
        eta_original = *ETA_REF;
    }

    //*=============================================* 
    //* 5) Calculate eta min/max also create limits *
    //*    for eta histogram bins                   *
    //*=============================================*
    ...
    *ETA_MAX = *(answer + 1);
    ...
    //*========================*
    //* 6) Simulate Evolution  *
    //*========================*
    //if(evol1_mcmc0){
    Evolve_Sequence(&Sequence, aa_count, i, RESIDENCE_TIMES_SIM_GENOTYPES, ETA_SIM_GENOTYPES, MU_RATIO_SIM_OVER_OBS, UNSCALED_PR_OBS_GENOTYPE, ETA_REF, bin_lims, BINS, NUM_BINS, BURN_IN_ETA, BURN_IN_TIME);
	//}else{
	//MCMC_Sequence(&Sequence, aa_count, i, RESIDENCE_TIMES_SIM_GENOTYPES, ETA_SIM_GENOTYPES, MU_RATIO_SIM_OVER_OBS, UNSCALED_PR_OBS_GENOTYPE, ETA_REF, bin_lims, BINS, NUM_BINS, BURN_IN_ETA, BURN_IN_TIME);
	//}	
    //*=========================*
    //* 7) Update *CODON_INDEX  *
    //*=========================*
 ...
 *========================================*/
 ...
 void calc_exact(int *CODON_INDEX,                  int *AA_COUNT,                    double *PHI,
    ...
    *ETA_MAX = *(answer + 1);
    ...
    Convert_Codon_Index_to_Sigma_Vec(Sequence.codon_index,Sequence.sigma_vec,aa_count);
    Sequence.sigma_obs = Sequence.sigma_vec[aa_count-1];
    Sequence.xi_obs = Calc_Xi_NSE(Sequence.sigma_vec,aa_count);          
    Sequence.eta_obs = Calc_Eta_NSE(Sequence.sigma_obs,Sequence.xi_obs,aa_count);
    ...
    for(i=0;i<size_codon_space;i++) {
        ...
        //* 5.1) Codon Index -> Sigma Vec
        //*      Converts t_codon_index to its vector of elongation probabilities
        //*      t_sigma_vec[i] corresponds to the translation probability up to 
        //*      and including i. sigma_n corresponds to the translatin probability
        //*      of the entire gene.
          
          Convert_Codon_Index_to_Sigma_Vec (t_codon_index,
                            t_sigma_vec,
                            aa_count);

          t_sigma_n = t_sigma_vec[aa_count - 1];

        
        //* 5.2) Calculate Xi and Eta 
        //*      Math can be found in ces2/source/data_simulation/CES.DATA.SIM.README.pdf
        
          t_xi = Calc_Xi_NSE (t_sigma_vec, aa_count);

          t_eta = Calc_Eta_NSE (t_sigma_n, t_xi, aa_count);
...
void Calc_Eta_NSE_R_wrapper (int * CODON_INDEX, int * AA_COUNT, double * ELONG_PR, int * N_CODONS, double * A_1, double * A_2,double * ETA){
	double xi, eta;
	int i;
	...
	Convert_Codon_Index_to_Sigma_Vec(Sequence.codon_index,Sequence.sigma_vec,Sequence.aa_count);
    Sequence.sigma_obs = Sequence.sigma_vec[Sequence.aa_count - 1];
     
    Sequence.xi_obs = Calc_Xi_NSE(Sequence.sigma_vec,Sequence.aa_count);
    Sequence.eta_obs = Calc_Eta_NSE(Sequence.sigma_obs,Sequence.xi_obs,Sequence.aa_count);
	Rprintf("eta: %f\n",Sequence.eta_obs);
	*ETA = Sequence.eta_obs;
...
void eta_min_max(int *aa_index,int aa_count, double * answer){
    ...
    for(i=0;i<aa_count;i++){
        ...
        if(pess_codon_index[i]==25443||opt_codon_index[i]==25443){ //25443 is the integer equivalent of the character 99
    ...
    Convert_Codon_Index_to_Sigma_Vec(opt_codon_index,t_sigma_vec_min,aa_count);
    
    t_sigma_n = t_sigma_vec_min[aa_count - 1];
    
    t_xi = Calc_Xi_NSE(t_sigma_vec_min,aa_count);
    
    eta_min = Calc_Eta_NSE(t_sigma_n,t_xi,aa_count);
        
    //*==========================*
    //* 3) Calculate maximum eta *
    //*==========================*
    
    Convert_Codon_Index_to_Sigma_Vec(pess_codon_index,t_sigma_vec_max,aa_count);
        
    t_sigma_n = t_sigma_vec_max[aa_count - 1];
        
    t_xi = Calc_Xi_NSE(t_sigma_vec_max,aa_count);
        
    eta_max = Calc_Eta_NSE(t_sigma_n,t_xi,aa_count);
